...
class SeasonWeeksTest : BehaviorSpec({
    given("a league competition finishing before the cup with a break") {
        ...
        `when`("the season weeks are calculated") {
            ...
            Then("there should be a single break") {
                seasonWeeks.breakWeeks() shouldContainExactly mapOf(Pair(getMsTime("08/09/2025"), "Break 1"))
            }
            Then("the league season should exclude the break") {
                seasonWeeks.competitionWeeks(1.toShort()) shouldContainExactly listOf(getMsTime("01/09/2025"), getMsTime("15/09/2025"))
            }
            then("The cup competition should be consecutive weeks") {
                seasonWeeks.competitionWeeks(2.toShort()) shouldContainExactly listOf(getMsTime("15/09/2025"), getMsTime("22/09/2025"), getMsTime("29/09/2025"))
            }
        }
    }
    ...
    given("a cup competition with a start date of 0 and valid league competition") {
        ...
        `when`("the season weeks are calculated") {
            ...
            Then("the league season should exclude the break") {
                seasonWeeks.competitionWeeks(1.toShort()) shouldContainExactly listOf(getMsTime("01/09/2025"), getMsTime("08/09/2025"), getMsTime("15/09/2025"))
            }
        }
    }
    ...
    given("a long season with several breaks") {
        val seasonCompetitions = listOf(createSeasonCompetition(1, "01/09/2025", "09/04/2026"))
        val breaks = listOf(createSeasonBreak(1, "20/10/2025"),
            createSeasonBreak(2, "15/12/2025"),
            createSeasonBreak(3, "22/12/2025"),
        ...
        When("The season weeks are calculated") {
            ...
            Then("Breaks should contain the 4 dates") {
                seasonWeeks.breakWeeks() shouldContainExactly mapOf(Pair(getMsTime("20/10/2025"), "Break 1"),
                    Pair(getMsTime("15/12/2025"), "Break 2"),
                    Pair(getMsTime("22/12/2025"), "Break 3"),
                    Pair(getMsTime("23/02/2026"), "Break 4"))
...
private fun createSeasonBreak(id: Int, date: String): SeasonBreak = SeasonBreak(id.toShort(), 1.toShort(), "Break $id", getMsTime(date))
