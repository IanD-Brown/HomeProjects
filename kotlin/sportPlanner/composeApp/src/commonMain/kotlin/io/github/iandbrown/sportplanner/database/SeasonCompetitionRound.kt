...
interface SeasonCompetitionRoundDao : BaseSeasonCompReadDao<SeasonCompetitionRound>, BaseWriteDao<SeasonCompetitionRound> {
    ...
    @Query("SELECT * FROM $table scr WHERE seasonId = :seasonId " +
            "AND NOT EXISTS (SELECT * FROM SeasonCupFixtures scf WHERE scf.seasonId = scr.seasonId AND scf.competitionId = scr.competitionId AND scf.round = scr.round AND scf.awayAssociationId <> 0 AND scf.result <> 0)")
