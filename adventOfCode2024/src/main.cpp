...
static void invoke(solver *day, solveResult t, function<bool(solveResult)> pt1,
                   solveResult t2, function<bool(solveResult)> pt2) {
	day->loadTestData();
...
static void invoke(solver *day, string t, string pt1, string t2, string pt2) {
	day->loadTestData();
...
int main() {
   invoke(new day1Solver("day1\\data"), 11, eq(1590491LL), 31, eq(22588371LL));
	...
	invoke(new day8Solver("day8\\data.txt"), 14, eq(311), 34, eq(1115));
	...
	invoke(new day25Solver("day25\\data.txt"), 3, eq(3196LL), -1, 0);
